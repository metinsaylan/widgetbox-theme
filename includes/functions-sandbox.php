...
function widget_sandbox_search($args) {
	...
	$title = empty($options['title']) ? __( 'Search', 'widgetbox' ) : attribute_escape($options['title']);
	$button = empty($options['button']) ? __( 'Find', 'widgetbox' ) : attribute_escape($options['button']);
?>
...
?>
	<p><label for="search-title"><?php _e( 'Title:', 'widgetbox' ) ?> <input class="widefat" id="search-title" name="search-title" type="text" value="<?php echo $title; ?>" /></label></p>
	<p><label for="search-button"><?php _e( 'Button Text:', 'widgetbox' ) ?> <input class="widefat" id="search-button" name="search-button" type="text" value="<?php echo $button; ?>" /></label></p>
...
function widget_sandbox_meta($args) {
	...
	$title = empty($options['title']) ? __( 'Meta', 'widgetbox' ) : attribute_escape($options['title']);
?>
			<?php echo $before_widget; ?>
...
function widget_sandbox_rsslinks($args) {
	...
	$title = empty($options['title']) ? __( 'RSS Links', 'widgetbox' ) : attribute_escape($options['title']);
?>
		<?php echo $before_widget; ?>
			<?php echo $before_title . $title . $after_title; ?>
			<ul>
				<li><a href="<?php bloginfo('rss2_url') ?>" title="<?php echo wp_specialchars( get_bloginfo('name'), 1 ) ?> <?php _e( 'Posts RSS feed', 'widgetbox' ); ?>" rel="alternate" type="application/rss+xml"><?php _e( 'All posts', 'widgetbox' ) ?></a></li>
				<li><a href="<?php bloginfo('comments_rss2_url') ?>" title="<?php echo wp_specialchars(bloginfo('name'), 1) ?> <?php _e( 'Comments RSS feed', 'widgetbox' ); ?>" rel="alternate" type="application/rss+xml"><?php _e( 'All comments', 'widgetbox' ) ?></a></li>
			</ul>
		<?php echo $after_widget; ?>
<?php
}
...
function widget_sandbox_rsslinks_control() {
	...
	$title = attribute_escape($options['title']);
?>
	<p><label for="rsslinks-title"><?php _e( 'Title:', 'widgetbox' ) ?> <input class="widefat" id="rsslinks-title" name="rsslinks-title" type="text" value="<?php echo $title; ?>" /></label></p>
...
function sandbox_widgets_init() {
	...
	wp_register_sidebar_widget( 'search', __( 'Search', 'widgetbox' ), 'widget_sandbox_search', $widget_ops );
	unregister_widget_control('search'); // We're being Sandbox-specific; remove WP default
	wp_register_widget_control( 'search', __( 'Search', 'widgetbox' ), 'widget_sandbox_search_control' );
	...
	wp_register_sidebar_widget( 'meta', __( 'Meta', 'widgetbox' ), 'widget_sandbox_meta', $widget_ops );
	unregister_widget_control('meta'); // We're being Sandbox-specific; remove WP default
	wp_register_widget_control( 'meta', __( 'Meta', 'widgetbox' ), 'wp_widget_meta_control' );
	...
	wp_register_sidebar_widget( 'rss_links', __( 'RSS Links', 'widgetbox' ), 'widget_sandbox_rsslinks', $widget_ops );
	wp_register_widget_control( 'rss_links', __( 'RSS Links', 'widgetbox' ), 'widget_sandbox_rsslinks_control' );
}
*/

// Translate, if applicable
load_theme_textdomain('widgetbox');
